    ...
    const IBSU_SegmentPosition* pSegmentPositionArray)
{
    ...
    // NFIQ calculating score
    if (segmentImageArrayCount >= 1)
    {
        int scoreArray[4] = {0};  
        char scoreString[256] = {0};
        int scorePos = 0; 
        ...
        for (int i = 0; i < segmentImageArrayCount; i++)
        {
            ...
            int score = -1;
            int result = IBSU_NFIQ2_ComputeScore(
                (BYTE*)(pSegmentImageArray+i)->Buffer, 
                (pSegmentImageArray+i)->Width, (pSegmentImageArray+i)->Height, (pSegmentImageArray+i)->BitsPerPixel, &score);
            if (result == IBSU_NFIQ2_STATUS_OK) {
                //std::cout << "\nNFIQ2 Score = " << score << std::endl;
                scoreArray[i] = score;
            } else {
                std::cerr << "[Error] Failed to compute NFIQ2 score." << std::endl;
                return;
            }
        }
        ...
        {
            if (i == 0) {
                scorePos += sprintf(scoreString + scorePos, "%d", scoreArray[i]);
            } else {
                scorePos += sprintf(scoreString + scorePos, "-%d", scoreArray[i]);
            }
        }
        printf("[NFIQ2] Score : %s",scoreString);
...
static BOOL DisplayMenu()
{
    ...
    printf("\nReady.   Enter choice:\n" \
           ...
           "\t5. Start capture for upper palm\n" \
           "\t6. Start capture for lower palm\n" \
           "\t7. Start capture for writer's palm\n" \
           "\t===============================================\n" \
           "\ta. Abort Capture\n" \
           "\tb. Options\n" \
           "\tc. Set Preview Image\n" \
           "\td. Set Spoof function control\n" \
           "\te. End program\n:");
     ...
     switch(ch) 
     {
        ...
        case '5': // Upper palm 
            printf("==>%c\n", ch);
            ok = StartCapture(ENUM_IBSU_FLAT_SINGLE_UPPER_PALM);
            if (!ok) 
            {
                return FALSE;
            }
        ...
        case '6': // Lower palm
            printf("==>%c\n", ch);
            ok = StartCapture(ENUM_IBSU_FLAT_SINGLE_LOWER_PALM);
            if (!ok) 
            {
                return FALSE;
            }
        ...
        case '7': // Writer's palm
            printf("==>%c\n", ch);
            ok = StartCapture(ENUM_IBSU_FLAT_SINGLE_WRITERS_PALM);
            if (!ok) 
            {
                return FALSE;
            }
...
int main(int argc, char* argv[])
{
    ...
    /* Initialize NFIQ2 */ 
    printf("[NFIQ2] Initializing...\n");

    if (IBSU_NFIQ2_Initialize() != IBSU_NFIQ2_STATUS_OK) {
        std::cerr << "[Error] Failed to initialize NFIQ2." << std::endl;
        return 1;
    }
    printf("[NFIQ2] Initialized successfully\n");

    /* Print NFIQ2 Versions */
    if (IBSU_NFIQ2_GetVersion(nfiq2Version) == IBSU_NFIQ2_STATUS_OK)
        std::cout << "IB NFIQ2 Version: " << nfiq2Version << std::endl;

    if (IBSU_NFIQ2_GetNISTVersion(nfiq2Version) == IBSU_NFIQ2_STATUS_OK)
        std::cout << "NIST NFIQ2 Version: " << nfiq2Version << std::endl;
