 ...
 *     2021/02/17  Added wsqEncodeToMemNative() native method.
 *                 Added wsqDecodeToMemNative() native method.
	 ...
	 jobject  error)
{
    ...
    {
        devHandle = getHandle(pEnv, tthis);
		// Currently "ENUM_IBSU_PROPERTY_DR_MODE_ZOOM_IN" is now available for windows apps which uses dc handle.
		// For non-Windows app and Windows JAVA app working as DR ZOOM IN Mode only and
		// Customer won't turn off this Zoom in mode

		if ((IBSU_PropertyId)propertyIDCode == ENUM_IBSU_PROPERTY_DR_MODE_ZOOM_IN && strcmp(propertyValueC,"FALSE") == 0)
		{
			// For debug print message (Commented out)
			LOG(("[SetProperty] Property = ENUM_IBSU_PROPERTY_DR_MODE_ZOOM_IN / Value =  %s\n", propertyValueC));
	...
	jobject  error)
{
	...
	IBSU_ImageData          splitImageArray[IBSU_MAX_SEGMENT_COUNT];
	int                     splitImageArrayCount;
	IBSU_FingerCountState   fingerCountState;
	IBSU_FingerQualityState qualityArray[IBSU_MAX_SEGMENT_COUNT];
	...
	jobject  error)
{
	...
	IBSU_FingerCountState   fingerCountState;
	IBSU_FingerQualityState qualityArray[IBSU_MAX_SEGMENT_QUALITY_COUNT];
	int                     qualityArrayCount;
	...
	jobject  error)
{
	...
	IBSM_ImageData splitResultImages[IBSU_MAX_SEGMENT_COUNT];
...
JNIEXPORT jobjectArray JNICALL IBSCANDEVICE_BUILD_JNI_FNCT(wsqEncodeToMemNative)(
    JNIEnv    *pEnv,
    jobject    tthis,
	jbyteArray image,
	jint       width,
    ...
    jobject    error)
{
    ...
    commentTextC = (*pEnv)->GetStringUTFChars(pEnv, commentText, NULL);
	inImage = (unsigned char *)(*pEnv)->GetByteArrayElements(pEnv, image, NULL);
	...
	nRc = IBSU_WSQEncodeMem((BYTE*)inImage, width, height, pitch, bitPerPixel,
							  pixelPerInch, (double)0.75, "",
							  &outImage, &compressedLengthC);
	setNativeError(pEnv, error, nRc);

	// Release resources
	(*pEnv)->ReleaseByteArrayElements(pEnv, image, (jbyte *)inImage, 0);
	(*pEnv)->ReleaseStringUTFChars(pEnv, commentText, commentTextC);
	...
	if (nRc == IBSU_STATUS_OK)
	{
		if (outImage == NULL)
		{
			LOG(("%s: WSQ compressed image was empty\n", __FUNCTION__));
			setNativeError(pEnv, error, IBSU_ERR_INVALID_ACCESS_POINTER);
		}
		else
		{
			jbyteArray compressedWsqByteJ;
			compressedWsqByteJ = (*pEnv)->NewByteArray(pEnv,compressedLengthC);
			if (compressedWsqByteJ == NULL)
			{
				LOG(("%s: unable to allocate an array of objects\n", __FUNCTION__));
				setNativeError(pEnv, error, IBSU_ERR_MEM_ALLOC);
			}
			...
			{
				jobject compressedWsqLengthJ;

				(*pEnv)->SetByteArrayRegion(pEnv,compressedWsqByteJ,0,compressedLengthC,(jbyte*)outImage);

				compressedWsqLengthJ = (*pEnv)->NewObject(pEnv, Class_Integer, MethodID_Integer_Integer, (jint)compressedLengthC);
				if (compressedWsqLengthJ == NULL)
				{
					LOG(("%s: unable to create Integer object\n", __FUNCTION__));
					setNativeError(pEnv, error, IBSU_ERR_MEM_ALLOC);
				} 
				else
				{
					...
					{
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 0, compressedWsqByteJ);
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 1, compressedWsqLengthJ);
					}
				}
			}
		}
...
/*
 * private native int wsqDecodeToMemNative(..., NativeError error);
 */
JNIEXPORT jobjectArray JNICALL IBSCANDEVICE_BUILD_JNI_FNCT(wsqDecodeToMemNative)(
    JNIEnv    *pEnv,
    jobject    tthis,
	jbyteArray compressedImage,
	jint       compressedLength,
	jobject    error)
{
	int nRc = 0;
	int compressedLengthC = 0;
	int outWidth = 0, outHeight = 0, outPitch =0, outBitsPerPixel = 0, outPixelPerInch = 0;
	unsigned char *inImage; // compressed Image
	unsigned char *outImage; // decomperssed Image
	jobjectArray   returns = NULL;

	inImage = (unsigned char *)(*pEnv)->GetByteArrayElements(pEnv, compressedImage, NULL);

	nRc = IBSU_WSQDecodeMem(inImage, compressedLength, &outImage, &outWidth, &outHeight,
							  &outPitch, &outBitsPerPixel, &outPixelPerInch);

	setNativeError(pEnv, error, nRc);
	//Resource release
	(*pEnv)->ReleaseByteArrayElements(pEnv, compressedImage, (jbyte *)inImage, 0);

	if (nRc == IBSU_STATUS_OK)
	{
		if (outImage == NULL)
		{
			LOG(("%s: WSQ decompressed image was empty\n", __FUNCTION__));
			setNativeError(pEnv, error, IBSU_ERR_INVALID_ACCESS_POINTER);
		}
		else
		{
			jbyteArray decompressedWsqByteJ;
			decompressedWsqByteJ = (*pEnv)->NewByteArray(pEnv,(outWidth*outHeight));
			if (decompressedWsqByteJ == NULL)
			{
				LOG(("%s: unable to allocate an array of objects\n", __FUNCTION__));
				setNativeError(pEnv, error, IBSU_ERR_MEM_ALLOC);
			}
			...
			{
				jobject outWidthJ,outHeightJ, outPitchJ, outBitsPerPixelJ, outPixelPerInchJ;

				(*pEnv)->SetByteArrayRegion(pEnv,decompressedWsqByteJ,0,(outWidth*outHeight),(jbyte*)outImage);

				outWidthJ = (*pEnv)->NewObject(pEnv, Class_Integer, MethodID_Integer_Integer, (jint)outWidth);
				outHeightJ = (*pEnv)->NewObject(pEnv, Class_Integer, MethodID_Integer_Integer, (jint)outHeight);
				outPitchJ = (*pEnv)->NewObject(pEnv, Class_Integer, MethodID_Integer_Integer, (jint)outPitch);
				outBitsPerPixelJ = (*pEnv)->NewObject(pEnv, Class_Integer, MethodID_Integer_Integer, (jint)outBitsPerPixel);
				outPixelPerInchJ = (*pEnv)->NewObject(pEnv, Class_Integer, MethodID_Integer_Integer, (jint)outPixelPerInch);

				if (outWidthJ == NULL || outHeightJ == NULL)
				{
					LOG(("%s: unable to create Integer object\n", __FUNCTION__));
					setNativeError(pEnv, error, IBSU_ERR_MEM_ALLOC);
				} 
				...
				{
					returns = (*pEnv)->NewObjectArray(pEnv, 6, Class_Object, NULL);
					if (returns == NULL)
					{
						LOG(("%s: unable to allocate object array of length 2\n", __FUNCTION__));
						setNativeError(pEnv, error, IBSU_ERR_MEM_ALLOC);
					...
					{
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 0, decompressedWsqByteJ);
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 1, outWidthJ);
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 2, outHeightJ);
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 3, outPitchJ);
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 4, outBitsPerPixelJ);
						(*pEnv)->SetObjectArrayElement(pEnv, returns, 5, outPixelPerInchJ);
					}
				}
			}
		}
...
/*
 * private native int fwriteViaJNINative(..., NativeError error);
 */
JNIEXPORT jint JNICALL IBSCANDEVICE_BUILD_JNI_FNCT(SaveRAWImageNative)(
    JNIEnv    *pEnv,
    jobject    tthis,
    ...
    jobject    error)
{
	...
	buffer = (unsigned char *)(*pEnv)->GetByteArrayElements(pEnv, writeBuffer, NULL);
	if( buffer == NULL )
	{
		LOG(("%s: unable to convert File write buffer\n",__FUNCTION__));
		setNativeError(pEnv , error , IBSU_ERR_INVALID_ACCESS_POINTER);
		return -1;
	}
	fileNameC = (*pEnv)->GetStringUTFChars(pEnv,fileName,NULL);
	fp = fopen(fileNameC , "wb");
	if (fp == NULL)
	{
		LOG(("%s: unable to write buffer to file\n",__FUNCTION__));
		setNativeError(pEnv , error , IBSU_ERR_FILE_OPEN);
		return -1;
	}
	...
	(*pEnv)->ReleaseByteArrayElements(pEnv, writeBuffer, (jbyte *)buffer, 0);
	(*pEnv)->ReleaseStringUTFChars(pEnv,fileName,fileNameC);
	return nRc;
	...
	jobject  error)
{
	...
	IBSM_ImageData SegmentImageArray[IBSU_MAX_SEGMENT_COUNT];
